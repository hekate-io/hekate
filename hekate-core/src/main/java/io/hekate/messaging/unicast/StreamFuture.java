...
public class StreamFuture<T> extends MessagingFuture<List<Response<T>>> {
     ...
     */
    public List<T> getResponse(long timeout, TimeUnit unit) throws MessagingFutureException, InterruptedException, TimeoutException {
        return get(timeout, unit).stream().map(Response::get).collect(toList());
    }
     ...
     * @throws InterruptedException Signals that thread was interrupted while awaiting for operation completion.
     */
    public List<T> getResponse() throws MessagingFutureException, InterruptedException {
        return get().stream().map(Response::get).collect(toList());
    }
     ...
     * @throws MessagingFutureException Signals that request operation failed.
     */
    public List<T> getResponseUninterruptedly() throws MessagingFutureException {
        return getUninterruptedly().stream().map(Response::get).collect(toList());
    }
}
