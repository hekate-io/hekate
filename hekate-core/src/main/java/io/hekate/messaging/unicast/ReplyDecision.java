 ...
 *
 * @see RequestCallback#accept(Throwable, Object)
 */
public enum ReplyDecision {
    /**
     * Signals that operation result was accepted and {@link RequestCallback#onComplete(Throwable, Reply)} method should be called. Note
     ...
     * policy is not configured for the channel then {@link RequestCallback#onComplete(Throwable, Reply)} will be called with {@link
     ...
     * be applied (if configured for the channel) or {@link RequestCallback#onComplete(Throwable, Reply)} should be called otherwise.
     */
    DEFAULT
}
