...
public enum FailoverRoutingPolicy {
    /**
     * Do not try to re-route and always use the same {@link FailureInfo#getLastNode() node}.
     */
    RETRY_SAME_NODE,

    /**
     * If {@link FailureInfo#getLastNode() target node} is still within the cluster topology then try using it. If target node left the
